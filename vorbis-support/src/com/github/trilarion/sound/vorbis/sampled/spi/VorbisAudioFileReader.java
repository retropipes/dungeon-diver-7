...
public class VorbisAudioFileReader extends TAudioFileReader {
	...
	// New Patch from MS:
	AudioFormat format = new AudioFormat(VORBISENC, vorbisInfo.rate, AudioSystem.NOT_SPECIFIED, vorbisInfo.channels,
		1, frameRate, false, af_properties);
	// Patch end
	return new VorbisAudioFileFormat(OGG_AUDIOFILEFORMAT_TYPE, format, AudioSystem.NOT_SPECIFIED, mediaLength,
		aff_properties);
    }
	...
	for (byte[] ptr1 : ptr) {
	    ...
	    aff_properties.put("ogg.comment.encodedby",
		    new String(vorbisComment.vendor, 0, vorbisComment.vendor.length - 1, Charset.forName("US-ASCII")));
	}
    }
