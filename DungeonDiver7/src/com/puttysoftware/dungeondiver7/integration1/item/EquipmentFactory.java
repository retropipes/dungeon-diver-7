...
public class EquipmentFactory {
	...
	return new Equipment(
		WeaponMaterialConstants.getWeaponMaterial(material) + " " + WeaponConstants.getWeapon(weaponType),
		Shop.getEquipmentCost(material), material + 1, material + 1, EquipmentSlotConstants.SLOT_WEAPON,
	...
	return new Equipment(
		ArmorMaterialConstants.getArmorMaterial(material) + " " + ArmorConstants.getArmor(armorType),
		Shop.getEquipmentCost(material), material + 1, material + 1,
		EquipmentSlotConstants.getArmorSlotForType(armorType), material, -1);
    }

    public static String getWeaponName(final int zoneID, final int weaponType) {
	return WeaponMaterialConstants.getWeaponMaterial(zoneID) + " " + WeaponConstants.getWeapon(weaponType);
    }

    public static String getArmorName(final int zoneID, final int armorType) {
	return ArmorMaterialConstants.getArmorMaterial(zoneID) + " " + ArmorConstants.getArmor(armorType);
    }
}
