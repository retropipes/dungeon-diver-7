...
public abstract class AbstractDungeonObject extends CloneableObject {
	...
	public boolean rangeAction(final int locX, final int locY, final int locZ, final int dirX, final int dirY,
		...
		if (RangeTypeConstants.getMaterialForRangeType(rangeType) == MaterialConstants.MATERIAL_FIRE
			...
			DungeonDiver7.getApplication().getGameManager().morph(this.changesToOnExposure(MaterialConstants.MATERIAL_FIRE),
			...
			DungeonDiver7.getApplication().getGameManager().morph(this.changesToOnExposure(MaterialConstants.MATERIAL_ICE),
					locX + dirX, locY + dirY, locZ, this.getLayer());
			return true;
	...
	public Direction laserEnteredAction(final int locX, final int locY, final int locZ, final int dirX, final int dirY,
		...
		if (this.isSolid()) {
			if (forceUnits > this.getMinimumReactionForce() && this.canMove()) {
				try {
					final AbstractDungeonObject nextObj = DungeonDiver7.getApplication().getDungeonManager().getDungeon()
							.getCell(locX + dirX, locY + dirY, locZ, this.getLayer());
					final AbstractDungeonObject nextObj2 = DungeonDiver7.getApplication().getDungeonManager().getDungeon()
					...
					if (this instanceof AbstractMovableObject && nextObj != null
						...
						DungeonDiver7.getApplication().getGameManager().updatePushedPositionLater(locX, locY, dirX, dirY,
