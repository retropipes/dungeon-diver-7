...
public final class CurrentDungeonData extends AbstractDungeonData {
	...
	@Override
	public AbstractDungeonObject getVirtualCell(final AbstractDungeon dungeon, final int row, final int col, final int floor,
			final int layer) {
		int fR = row;
		int fC = col;
		int fF = floor;
	...
	@Override
	public int[] circularScanTunnel(final AbstractDungeon dungeon, final int xIn, final int yIn, final int zIn, final int r,
	...
	@Override
	public void circularScanRange(final AbstractDungeon dungeon, final int xIn, final int yIn, final int zIn, final int r,
		...
		for (u = xFix - r; u <= xFix + r; u++) {
			for (v = yFix - r; v <= yFix + r; v++) {
				...
				for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
					try {
						this.getCell(dungeon, u, v, zFix, w).rangeAction(xFix, yFix, zFix, u - xFix, v - yFix, rangeType,
								forceUnits);
					} catch (final ArrayIndexOutOfBoundsException aioob) {
						// Do nothing
					}
		...
		for (int x = 0; x < AbstractDungeonData.MIN_COLUMNS; x++) {
			for (int y = 0; y < AbstractDungeonData.MIN_ROWS; y++) {
				for (int z = 0; z < this.getFloors(); z++) {
					final AbstractDungeonObject obj = this.getCell(dungeon, y, x, z, DungeonConstants.LAYER_LOWER_OBJECTS);
	...
	public void fullScanFreezeGround(final AbstractDungeon dungeon) {
		...
		for (int x = 0; x < AbstractDungeonData.MIN_COLUMNS; x++) {
			for (int y = 0; y < AbstractDungeonData.MIN_ROWS; y++) {
				for (int z = 0; z < this.getFloors(); z++) {
					final AbstractDungeonObject obj = this.getCell(dungeon, y, x, z, DungeonConstants.LAYER_LOWER_GROUND);
	...
	@Override
	public void fullScanFindButtonLostDoor(final AbstractDungeon dungeon, final int zIn, final AbstractButtonDoor door) {
		// Perform the scan
		int zFix = zIn;
		if (dungeon.isThirdDimensionWraparoundEnabled()) {
			zFix = this.normalizeFloor(zFix);
	...
	@Override
	public void setVirtualCell(final AbstractDungeon dungeon, final AbstractDungeonObject mo, final int row, final int col,
			final int floor, final int layer) {
		int fR = row;
		int fC = col;
		int fF = floor;
	...
	public void restore(final AbstractDungeon dungeon) {
		...
		for (x = 0; x < this.getColumns(); x++) {
			for (y = 0; y < this.getRows(); y++) {
				for (z = 0; z < this.getFloors(); z++) {
					for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
						this.setCell(dungeon, (AbstractDungeonObject) this.savedState.getCell(x, y, z, w).clone(), y, x, z,
								w);
					}
				}
			}
	...
	@Override
	public void fillNulls(final AbstractDungeon dungeon, final AbstractDungeonObject fill1, final AbstractDungeonObject fill2,
			final boolean was16) {
		int y, x, z, w;
		for (x = 0; x < this.getColumns(); x++) {
			for (y = 0; y < this.getRows(); y++) {
	...
	@Override
	public AbstractDungeonData readData(final AbstractDungeon dungeon, final XDataReader reader, final int formatVersion)
			throws IOException {
		...
		for (x = 0; x < dungeonSizeX; x++) {
			for (y = 0; y < dungeonSizeY; y++) {
				for (z = 0; z < dungeonSizeZ; z++) {
					final AbstractDungeonObject obj = DungeonDiver7.getApplication().getObjects().readDungeonObjectG2(reader,
							ver);
	...
	private static CurrentDungeonData readDataG3(final AbstractDungeon dungeon, final XDataReader reader, final int ver)
		...
		for (x = 0; x < dungeonSizeX; x++) {
			for (y = 0; y < dungeonSizeY; y++) {
				for (z = 0; z < dungeonSizeZ; z++) {
					final AbstractDungeonObject obj = DungeonDiver7.getApplication().getObjects().readDungeonObjectG3(reader,
							ver);
					lt.setCell(dungeon, obj, y, x, z, obj.getLayer());
				}
			}
	...
	private static CurrentDungeonData readDataG4(final AbstractDungeon dungeon, final XDataReader reader, final int ver)
		...
		for (x = 0; x < dungeonSizeX; x++) {
			for (y = 0; y < dungeonSizeY; y++) {
				for (z = 0; z < dungeonSizeZ; z++) {
					final AbstractDungeonObject obj = DungeonDiver7.getApplication().getObjects().readDungeonObjectG4(reader,
							ver);
					lt.setCell(dungeon, obj, y, x, z, obj.getLayer());
				}
			}
	...
	private static CurrentDungeonData readDataG5(final AbstractDungeon dungeon, final XDataReader reader, final int ver)
		...
		for (x = 0; x < dungeonSizeX; x++) {
			for (y = 0; y < dungeonSizeY; y++) {
				for (z = 0; z < dungeonSizeZ; z++) {
					for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
						lt.setCell(dungeon, DungeonDiver7.getApplication().getObjects().readDungeonObjectG5(reader, ver), y, x,
								z, w);
					}
				}
			}
	...
	private static CurrentDungeonData readDataG6(final AbstractDungeon dungeon, final XDataReader reader, final int ver)
		...
		for (x = 0; x < dungeonSizeX; x++) {
			for (y = 0; y < dungeonSizeY; y++) {
				for (z = 0; z < dungeonSizeZ; z++) {
					for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
						lt.setCell(dungeon, DungeonDiver7.getApplication().getObjects().readDungeonObjectG6(reader, ver), y, x,
								z, w);
					}
				}
			}
			...
			for (y = 0; y < saveSizeY; y++) {
				for (z = 0; z < saveSizeZ; z++) {
					this.savedState.setCell(
							DungeonDiver7.getApplication().getObjects().readDungeonObjectG2(reader, formatVersion), y, x, z,
							DungeonConstants.LAYER_LOWER_GROUND);
				}
			}
		}
	...
	private void readSavedStateG3(final XDataReader reader, final int formatVersion) throws IOException {
		...
		for (x = 0; x < saveSizeX; x++) {
			for (y = 0; y < saveSizeY; y++) {
				for (z = 0; z < saveSizeZ; z++) {
					this.savedState.setCell(
							DungeonDiver7.getApplication().getObjects().readDungeonObjectG3(reader, formatVersion), y, x, z,
							DungeonConstants.LAYER_LOWER_GROUND);
				}
			}
		}
	...
	private void readSavedStateG4(final XDataReader reader, final int formatVersion) throws IOException {
		...
		for (x = 0; x < saveSizeX; x++) {
			for (y = 0; y < saveSizeY; y++) {
				for (z = 0; z < saveSizeZ; z++) {
					this.savedState.setCell(
							DungeonDiver7.getApplication().getObjects().readDungeonObjectG4(reader, formatVersion), y, x, z,
							DungeonConstants.LAYER_LOWER_GROUND);
				}
			}
		}
		...
		for (x = 0; x < saveSizeX; x++) {
			for (y = 0; y < saveSizeY; y++) {
				for (z = 0; z < saveSizeZ; z++) {
					for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
						this.savedState.setCell(
								DungeonDiver7.getApplication().getObjects().readDungeonObjectG5(reader, formatVersion), y, x,
								z, w);
					}
				}
			}
		...
		for (x = 0; x < saveSizeX; x++) {
			for (y = 0; y < saveSizeY; y++) {
				for (z = 0; z < saveSizeZ; z++) {
					for (w = 0; w < DungeonConstants.NUM_LAYERS; w++) {
						this.savedState.setCell(
								DungeonDiver7.getApplication().getObjects().readDungeonObjectG6(reader, formatVersion), y, x,
								z, w);
					}
				}
			}
