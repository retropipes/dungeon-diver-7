...
public class AvatarImageCache {
	...
	if (!AvatarImageCache.isInCache(name)) {
	    final BufferedImageIcon bii = AvatarImageManager.getUncachedImage(name);
	...
	final CacheEntry[] tempCache = new CacheEntry[AvatarImageCache.cache.length + AvatarImageCache.CACHE_INCREMENT];
	...
	AvatarImageCache.cache[AvatarImageCache.CACHE_SIZE] = new CacheEntry(bii, name);
