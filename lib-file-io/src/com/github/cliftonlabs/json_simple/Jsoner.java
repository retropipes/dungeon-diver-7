 ...
 * thread safe parsing (RFC 7159) JSON strings, and thread safe serializing data
 * to strings in JSON format.
 * 
 * @since 2.0.0
 */
public class Jsoner {
	 ...
	 * serialization by serializing the non-JSON value directly into the now invalid
	 * JSON. Be mindful that invalid JSON will not successfully deserialize.
	 */
	ALLOW_INVALIDS,
	/**
	 * Instead of aborting serialization on non-JSON values that implement Jsonable
	 * it will continue serialization by deferring serialization to the Jsonable.
	 * 
	 * @see Jsonable
	 */
	ALLOW_JSONABLES;
     ...
     *                               JSON.
     * @return either a boolean, null, Number, String, JsonObject, or JsonArray that
     *         best represents the deserializable.
     * @throws JsonException if an unexpected token is encountered in the
     *                       deserializable. To recover from a JsonException: fix
     *                       the deserializable to no longer have an unexpected
     *                       token and try again.
     */
     ...
     * @throws JsonException if a disallowed or unexpected token is encountered in
     *                       the deserializable. To recover from a JsonException:
     *                       fix the deserializable to no longer have a disallowed
     *                       or unexpected token and try again.
     */
	...
	do {
		...
		/*
		 * Increment the amount of returned JSON values and treat the token as if it
		 * were a fresh parse.
		 */
		returnCount += 1;
	    ...
	    case PARSED_ERROR:
		/*
		 * The parse could be in this state due to the state stack not having a state to
		 * pop off.
		 */
		throw new JsonException(lexer.getPosition(), JsonException.Problems.UNEXPECTED_TOKEN, token);
	    case PARSING_ARRAY:
		...
		case COMMA:
		    /*
		     * The parse could detect a comma while parsing an object since it separates
		     * each key value pair. Continue parsing the object.
		     */
		    stateStack.addLast(currentState);
		    break;
		case DATUM:
			...
			/*
			 * JSON keys are always strings, strings are not always JSON keys but it is
			 * going to be treated as one. Continue parsing the object.
			 */
		...
		case COLON:
		    /*
		     * The parse could detect a colon while parsing a key value pair since it
		     * separates the key and value from each other. Continue parsing the entry.
		     */
		    stateStack.addLast(currentState);
		    break;
		case DATUM:
	...
	try {
	     ...
	     * deserializable is null, it should be reasonable to expect null back.
	     */
	    returnable = null;
	} finally {
     ...
     *                       isn't a JsonArray or an IOException,
     *                       NullPointerException, or JsonException occurs during
     *                       deserialization.
     * @return a JsonArray that represents the deserializable, or the defaultValue
     *         if there isn't a JsonArray that represents deserializable.
     * @see Jsoner#deserialize(Reader)
     */
     ...
     *                       JsonObject.
     * @param defaultValue   representing what would be returned if deserializable
     *                       isn't a JsonObject or an IOException,
     *                       NullPointerException, or JsonException occurs during
     *                       deserialization.
     * @return a JsonObject that represents the deserializable, or the defaultValue
     *         if there isn't a JsonObject that represents deserializable.
     * @see Jsoner#deserialize(Reader)
     */
    ...
    /**
     * A convenience method that assumes multiple RFC 7159 JSON values (except
     * numbers) have been concatenated together for deserilization which will be
     * collectively returned in a JsonArray wrapper. There may be numbers included,
     * they just must not be concatenated together as it is prone to
     * NumberFormatExceptions (thus causing a JsonException) or the numbers no
     * longer represent their respective values. Examples: "123null321" returns
     * [123, null, 321] "nullnullnulltruefalse\"\"{}[]" returns [null, null, null,
     * true, false, "", {}, []] "123" appended to "321" returns [123321] "12.3"
     * appended to "3.21" throws JsonException(NumberFormatException) "123" appended
     * to "-321" throws JsonException(NumberFormatException) "123e321" appended to
     * "-1" throws JsonException(NumberFormatException) "null12.33.21null" throws
     * JsonException(NumberFormatException)
     * 
     * @param deserializable representing concatenated content to be deserialized as
     *                       JSON in one reader. Its contents may not contain two
     ...
     * @return an escaped string for usage in JSON; An escaped string is one that
     *         has escaped all of the quotes ("), backslashes (\), return character
     *         (\r), new line character (\n), tab character (\t), backspace
     *         character (\b), form feed character (\f) and other control characters
     *         [u0000..u001F] or characters [u007F..u009F], [u2000..u20FF] with a
     *         backslash (\) which itself must be escaped by the backslash in a java
     *         string.
     */
	...
	for (int i = 0; i < characters; i++) {
	    ...
	    default:
		/*
		 * The many characters that get replaced are benign to software but could be
		 * mistaken by people reading it for a JSON relevant character.
		 */
     ...
     * 
     * @param lexer represents a text processor being used in the deserialization
     *              process.
     * @return a token representing a meaningful element encountered by the lexer.
     * @throws JsonException if an unexpected character is encountered while
     *                       processing the text.
     */
    ...
    /**
     * Creates a new JsonKey that wraps the given string and value. This function
     * should NOT be used in favor of existing constants and enumerations to make
     ...
     *                   processing.
     * @return a state for deserialization context so it knows how to consume the
     *         next token.
     */
    ...
    /**
     * Makes the JSON input more easily human readable using indentation and newline
     * of the caller's choice. This means the validity of the JSON printed by this
     * method is dependent on the caller's choice of indentation and newlines.
     * 
     * @param readable    representing a JSON formatted string with out extraneous
     *                    characters, like one returned from
     ...
     *                    string. NOT validated as a proper indentation. It is
     *                    recommended to use tabs ("\t"), but 3 or 4 spaces are
     *                    common alternatives.
     * @param newline     representing the newline used to format the JSON string.
     *                    NOT validated as a proper newline. It is recommended to
     ...
     *        control.
     */
    public static void prettyPrint(final Reader readable, final Writer writable, final String indentation,
	    final String newline) throws IOException, JsonException {
     ...
     *         after '[', '{', ',' and before ']' '}' tokens in the JSON. It will
     *         return null if printable isn't a JSON string.
     */
	...
	try {
	    ...
	    /*
	     * See java.io.StringReader. See java.io.StringWriter.
	     */
	} catch (final JsonException caught) {
	    /*
	     * Would have been caused by a an IO exception while lexing, but the
	     * StringReader does not throw them. See java.io.StringReader.
	     */
	}
     ...
     * 
     * @param jsonSerializable represents the object that should be serialized as a
     *                         string in JSON format.
     * @return a string, in JSON format, that represents the object provided.
     * @throws IllegalArgumentException if the jsonSerializable isn't serializable
     *                                  in JSON.
    ...
    /**
     * Serializes values according to the RFC 7159 JSON specification. It will also
     * trust the serialization provided by any Jsonables it serializes.
     * 
     * @param jsonSerializable    represents the object that should be serialized in
     *                            JSON format.
     * @param writableDestination represents where the resulting JSON text is
     *                            written to.
     * @throws IOException              if the writableDestination encounters an I/O
     *                                  problem, like being closed while in use.
     * @throws IllegalArgumentException if the jsonSerializable isn't serializable
     *                                  in JSON.
     */
     ...
     * behavior flags.
     * 
     * @param jsonSerializable    represents the object that should be serialized to
     *                            a string in JSON format.
     * @param writableDestination represents where the resulting JSON text is
     *                            written to.
     * @param flags               represents the allowances and restrictions on
     *                            serialization.
     * @throws IOException              if the writableDestination encounters an I/O
     *                                  problem.
     * @throws IllegalArgumentException if the jsonSerializable isn't serializable
	...
	if (jsonSerializable == null) {
	    ...
	    /*
	     * Make sure the string is properly escaped. Quotes for some reason are
	     * necessary for String, but not Character.
	     */
		...
		/*
		 * Infinite and not a number are not supported by the JSON specification, so
    ...
    /**
     * Serializes like the first version of this library. It has been adapted to use
     * Jsonable for serializing custom objects, but otherwise works like the old
     * JSON string serializer. It will allow non-JSON values in its output like the
     * old one. It can be helpful for last resort log statements and debugging
     * errors in self generated JSON. Anything serialized using this method isn't
     * guaranteed to be deserializable.
     * 
     * @param jsonSerializable    represents the object that should be serialized in
     *                            JSON format.
     * @param writableDestination represents where the resulting JSON text is
    ...
    /**
     * Serializes JSON values and only JSON values according to the RFC 7159 JSON
     * specification.
     * 
     * @param jsonSerializable    represents the object that should be serialized in
     *                            JSON format.
