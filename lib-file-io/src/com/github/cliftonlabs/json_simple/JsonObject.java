 ...
 * serialization. Meaning all values added to a JsonObject must be recognized by
 * the Jsoner for it to be a true 'JsonObject', so it is really a
 * JsonableHashMap that will serialize to a JsonObject if all of its contents
 * are valid JSON.
 * 
 * @see Jsoner
 * @since 2.0.0
...
public class JsonObject extends HashMap<String, Object> implements Jsonable {
    /**
     * The serialization version this class is compatible with. This value doesn't
     * need to be incremented if and only if the only changes to occur were updating
     * comments, updating javadocs, adding new fields to the class, changing the
     * fields from static to non-static, or changing the fields from transient to
     * non transient. All other changes require this number be incremented.
     */
    ...
    /**
     * Instantiate a new JsonObject by accepting a map's entries, which could lead
     * to de/serialization issues of the resulting JsonObject since the entry values
     * aren't validated as JSON values.
     * 
    ...
    /**
     * A convenience method that assumes there is a BigDecimal, Number, or String at
     * the given key. If a Number is there its Number#toString() is used to
     ...
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     *                               BigDecimal or if the Number represents the
     *                               double or float Infinity or NaN.
     * @see BigDecimal
     * @see Number#toString()
     * @see JsonKey
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a BigDecimal representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present.
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     *                               BigDecimal or if the Number represents the
    ...
    /**
     * A convenience method that assumes there is a Boolean or String value at the
     * given key.
     * 
     * @param key representing where the value ought to be paired with.
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Boolean representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present.
     * @throws ClassCastException if the value didn't match the assumed return type.
     * @see JsonKey
     * @since 2.3.0 to utilize JsonKey
    ...
    /**
     * A convenience method that assumes there is a Number or String value at the
     * given key.
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Byte representing the value paired with the key (which may involve
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Byte representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     ...
     * 
     * @param <T> the kind of collection to expect at the key. Note unless manually
     *            added, collection values will be a JsonArray.
     * @param key representing where the value ought to be paired with.
	...
	/*
	 * The unchecked warning is suppressed because there is no way of guaranteeing
	 * at compile time the cast will work.
	 */
     ...
     *            added, collection values will be a JsonArray.
     * @param key representing where the value ought to be paired with.
     * @return a Collection representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present..
     * @throws ClassCastException if the value didn't match the assumed return type.
     * @see JsonKey
     * @since 2.3.0 to utilize JsonKey
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Double representing the value paired with the key (which may
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Double representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Float representing the value paired with the key (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Float representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return an Integer representing the value paired with the key (which may
     *         involve rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return an Integer representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Long representing the value paired with the key (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Long representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     ...
     * 
     * @param <T> the kind of map to expect at the key. Note unless manually added,
     *            Map values will be a JsonObject.
     * @param key representing where the value ought to be paired with.
     ...
     *            Map values will be a JsonObject.
     * @param key representing where the value ought to be paired with.
     * @return a Map representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present.
     * @throws ClassCastException if the value didn't match the assumed return type.
     * @see JsonKey
     * @since 2.3.0 to utilize JsonKey
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Short representing the value paired with the key (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
     * @throws NumberFormatException if a String isn't a valid representation of a
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a Short representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present (which may involve
     *         rounding or truncation).
     * @throws ClassCastException    if the value didn't match the assumed return
     *                               type.
    ...
    /**
     * A convenience method that assumes there is a Boolean, Number, or String value
     * at the given key.
     * 
     * @param key representing where the value ought to be paired with.
     ...
     * 
     * @param key representing where the value ought to be paired with.
     * @return a String representing the value paired with the key or
     *         JsonKey#getValue() if the key isn't present.
     * @throws ClassCastException if the value didn't match the assumed return type.
     * @see JsonKey
     * @since 2.3.0 to utilize JsonKey
     ...
     * 
     * @param key   represents the JsonKey used for the value's association in the
     *              map.
    ...
    /**
     * Calls put for the given key and value, but returns the JsonObject for
     * chaining calls.
     * 
     ...
     * @return a boolean, which is true if the value was removed. It is false
     *         otherwise.
     * @since 3.1.1 to use JsonKey instead of calling JsonKey#getKey() each time.
