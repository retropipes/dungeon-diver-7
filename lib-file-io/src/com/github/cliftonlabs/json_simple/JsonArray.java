 ...
 * serialization. Meaning all values added to a JsonArray must be recognized by
 * the Jsoner for it to be a true 'JsonArray', so it is really a
 * JsonableArrayList that will serialize to a JsonArray if all of its contents
 * are valid JSON.
 * 
 * @see Jsoner
 * @since 2.0.0
...
public class JsonArray extends ArrayList<Object> implements Jsonable {
    ...
    /**
     * Calls add for the given collection of elements, but returns the JsonArray for
     * chaining calls.
     * 
    ...
    /**
     * Calls add for the given index and collection, but returns the JsonArray for
     * chaining calls.
     * 
    ...
    /**
     * Calls add for the given element, but returns the JsonArray for chaining
     * calls.
     * 
     * @param index   represents what index the element is added to in the
     *                JsonArray.
    ...
    /**
     * A convenience method that assumes every element of the JsonArray is castable
     * to T before adding it to a collection of Ts.
     * 
     * @param <T>         represents the type that all of the elements of the
     *                    JsonArray should be cast to and the type the collection
     *                    will contain.
     * @param destination represents where all of the elements of the JsonArray are
     *                    added to after being cast to the generic type provided.
     ...
     * the given index. If a Number or String is there it is used to construct a new
     * BigDecimal.
     * 
     ...
     *                                   assumed return type.
     * @throws NumberFormatException     if a String isn't a valid representation of
     *                                   a BigDecimal or if the Number represents
    ...
    /**
     * A convenience method that assumes there is a Collection value at the given
     * index.
     * 
